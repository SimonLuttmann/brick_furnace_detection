'''\
Sentinel-2 Kiln Detection  – 8‑Kanal‑Variante + WEIGHTED F1 SCORE
==================================================================
...
🔥 ENHANCED V4.1 - OPTIMIERTE ARCHITEKTUR & TRAINING + WEIGHTED F1 SCORE + DATA LEAKAGE FIX:
==========================================================================================
* **ERWEITERTE AUGMENTATIONS**: Spatial + Spectral für Sentinel-2
* **SELTENE KLASSEN BOOST**: Spezielle Augmentations für Klassen 4-8
* **VERBESSERTE DATASET-VALIDIERUNG**: Filtert kaputte Bilder vor Training
* **OPTIMIERTE NORMALISIERUNG**: Echte Dataset-Statistiken
* **ENHANCED UNET**: Bessere Gewichts-Initialisierung
* **ROBUSTE LOSS-FUNKTION**: CrossEntropyLoss mit class weights
* **INTELLIGENTE DATENLOADER**: Ausgewogene Klassen-Sampling
* **PERFORMANCE-MONITORING**: Tensorboard + Detailed Logging
* **WEIGHTED F1 SCORE**: Evaluation nach jeder Epoch + finaler Report
* **DATA LEAKAGE FIX**: Proper train/val split ohne Überlappung (80/20)
* **FAIR EVALUATION**: Validation ohne Augmentation für echte Performance
* **SELECTIVE FILTERING**: Nur Training gefiltert, Validation ungefiltert für realistische Evaluation
'''
...
def build_augmented_loaders():
    """Build data loaders with advanced augmentation strategy - FIXED DATA LEAKAGE + SELECTIVE FILTERING"""
    ...
    # Split ALL images first to avoid data leakage (80/20 split)
    ...
    # STEP 4: Create datasets with different configurations
    # Training dataset WITH augmentation, filtering, and higher positive ratio
    train_ds = SentinelKilnDataset.__new__(SentinelKilnDataset)  # Create without __init__
    train_ds.img_paths = train_img_paths_filtered
    train_ds.mask_dir = MASK_DIR
    train_ds.patch_size = PATCH_SIZE
    train_ds.positive_ratio = 0.7
    train_ds.augmentations = augmentations
    
    # Manual initialization for training dataset
    print("Initializing training dataset...")
    train_ds._cache_kiln_locations()
    train_ds.mean, train_ds.std = train_ds._compute_dataset_statistics()
    train_ds.norm = T.Normalize(mean=train_ds.mean, std=train_ds.std)
    
    # Validation dataset WITHOUT augmentation, filtering, and lower positive ratio
    val_ds = SentinelKilnDataset.__new__(SentinelKilnDataset)  # Create without __init__
    val_ds.img_paths = val_img_paths
    val_ds.mask_dir = MASK_DIR
    val_ds.patch_size = PATCH_SIZE
    val_ds.positive_ratio = 0.3
    val_ds.augmentations = None
    
    # Manual initialization for validation dataset
    print("Initializing validation dataset...")
    val_ds._cache_kiln_locations()
    val_ds.mean, val_ds.std = val_ds._compute_dataset_statistics()
    val_ds.norm = T.Normalize(mean=val_ds.mean, std=val_ds.std)
...
if len(all_final_preds) > 0:
    ...
    weighted_f1 = f1_score(all_final_labels, all_final_preds, average='weighted', zero_division=0)
    macro_f1 = f1_score(all_final_labels, all_final_preds, average='macro', zero_division=0)
